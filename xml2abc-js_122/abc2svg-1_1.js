...
// Copyright (C) 2014-2026 Jean-FranÃ§ois Moine - LGPL3+
// abc2svg - abc2svg.js
...
abc2svg.ch_alias={"maj":"","min":"m","-":"m","Â°":"dim","+":"aug","+5":"aug","maj7":"M7","Î”7":"M7","Î”":"M7","min7":"m7","-7":"m7","Ã¸7":"m7b5","Â°7":"dim7","min+7":"m+7","aug7":"+7","7+5":"+7","7#5":"+7","sus":"sus4","7sus":"7sus4"}
abc2svg.font_tb=[]
abc2svg.font_st={}
abc2svg.hdn={}
...
function sh_st(){if(dd.name=="ped)")
return
var de3,de2=de.start,s=de2.s,i=de2.ix
...
dd.str=nm
c=nm.slice(-1)
if(c=='('||(c==')'&&nm.indexOf('(')<0)){nm2=nm.slice(0,-1)+(c=='('?')':'(');dd2=dd_tb[nm2]
...
if(s.notes[0].acc)
continue
court=curvoice.acc[s.notes[0].pit+19]||3
break
...
s.a_dd=[]
s.a_dd.push(dd)}
if(court){a_dcn.push("cacc"+court)
dh_cnv(s,s.notes[0])}}
...
if(s.second||!gene.st_print[st]||!staff_tb[s.st].topbar)
break
...
delete parse.pq
init_tune()
img.chg=true;set_page();if(cfmt.show_source&&user.img_out){user.img_out("</div>")
...
parse.eol=eof
continue}
switch(a[1]){case"show_source":if(!user.img_out)
continue
b=uncomment(a[2])
...
sav.char_tb=clone(char_tb);sav.glovar=clone(glovar);sav.maps=clone(maps,1);sav.mac=clone(mac);sav.maci=clone(maci);if(cfmt.show_source&&user.img_out){bol-=2
...
function adjoct(s){var d,g,m
if(s.clef_oct_transp==voice_tb[s.v+1].clef.clef_oct_transp&&s.clef_octave==voice_tb[s.v+1].clef.clef_octave)
return
s=(s.clef_octave&&!s.clef_oct_transp)?s.p_v.sym:voice_tb[s.v+1].sym
d=s.p_v.clef.clef_octave
s.p_v.clef.clef_octave=null
while(s){if(s.type==C.NOTE){for(m=0;m<=s.nhd;m++)
s.notes[m].pit+=d}else if(s.type==C.GRACE){for(g=s.extra;g;g=g.next)
for(m=0;m<=g.nhd;m++)
g.notes[m].pit+=d}
s=s.next}}
...
if(s.clef_octave)
if(cfmt.sound){if(!s.clef_oct_transp)
...
blkdiv=2
break
case"sep":vskip(s.sk1);output+='<path class="stroke"\n\td="M';out_sxsy((get_lwidth()-s.l)/2,' ',0)
...
function new_bar(){var s2,c,bar_type,i,line=parse.line,s={type:C.BAR,fname:parse.fname,istart:parse.bol+line.index,dur:0,multi:0}
if(vover&&vover.bar)
get_vover('|')
...
while(--i>=0){if(!curvoice.sls[i].rep)
...
for(i=0;i<curvoice.sls.length;i++){if(!curvoice.sls[i].rep)
curvoice.sls[i].rep=1}}else{curvoice.tie_s=curvoice.tie_s_rep
...
if(sl.ss==s)
continue
if(sl.rep){if(sl.rep>=10)
break
sl.rep=10}else{curvoice.sls.splice(i,1)}
sl.se=s
if(nt)
sl.nte=nt
s2=sl.ss
...
s2.sls=[];s2.sls.push({ty:C.SL_AUTO,ss:s2,se:s,slr:sl})
if(nt)
...
s.nmeas=0}else{glovar.mrest_p=true
...
return r}
function parse_mac(k,m,b){var te,ti,curv,line_sav=line,istart_sav=parse.istart,s=curvoice.last_sym
if(!s&&cfmt.writefields.indexOf('m')<0)
return syntax(1,"Bad length of the macro sequence")
parse.line=line=new scanBuf;parse.istart+=line_sav.index;if(cfmt.writefields.indexOf('m')<0){line.buffer=k.replace('n',n2n(b))
ti=curvoice.time
parse_seq(true)
if(!s)
s=curvoice.sym
...
for(j=0;j<words.length;j++){ww=strwh(words[j]+'Â ')
w+=ww[0]
...
if(tgl.sc){out_XYAB('<text transform="translate(X,Y) scale(A)">B</text>\n',x,y,tgl.sc,tgl.c)}else if(tgl.c.length>1){out_XYAB('<text x="X" y="Y">A</text>\n',x,y,tgl.c)}else{gla[0].push(sx(x))
gla[1].push(sy(y))
...
function b_chk(){var bt,s,s2,s3,v,t,ir=0
while(1){v=vn[ir++]
if(v==undefined)
break
s=vtb[v]
...
continue
if(!bt){s3=s
bt=s.bar_type}else if(s.bar_type!=bt||s.text!=s3.text){break}}
if(v==undefined)
return
if(bt[0]==':'&&s.bar_type[0]==':'&&(bt=="::"||s.bar_type=='::')){ir=0
while(1){v=vn[ir++]
if(v==undefined)
return
s=vtb[v]
...
if(s.bar_type[0]!=':')
break
if(s.bar_type!='::'){if(s.bar_type!=":|")
break
s2=s
while(s2.next&&s2.next.time==s3.time){s2=s2.next
if(s2.bar_type)
break}
if(s2.bar_type=="|:")
continue
...
s2.next.prev=s2
s2.prev=s
s.next=s2}}else if(s3.text||s.text){if(!s3.text)
s3=s
t=s3.text
ir=0
while(1){v=vn[ir++]
...
if(!s||s.invis)
continue
if(s.text){if(s.text==t)
continue
break}
s2=s
while(s2.next&&s2.next.time==s.time){s2=s2.next
if(s2.text)
break}
...
s2.prev=s
s.next=s2}}
error(1,s3,"Different bars $1 and $2",(bt+(s3.text||'')),(s.bar_type+(s.text||'')))}
...
break}
continue
case C.MREST:if(voice_tb.length==1&&s.tacet&&s.invis)
s.invis=0
break
...
if(p_v.time>=mxt)
return
var p_v_sav=curvoice,dur=mxt-p_v.time,s={type:C.MREST,stem:0,multi:0,nhd:0,xmx:0,frm:1,dur:dur,dur_orig:dur,nmeas:dur/p_v.wmeasure,notes:[{pit:18,dur:dur}],tacet:p_v.tacet},s2={type:C.BAR,bar_type:'|',dur:0,multi:0}
...
abc2svg.version="v1.23.3-10-b850fac358";abc2svg.vdate="2026-07-19"
